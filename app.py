...
def rank_and_explain_jobs(jobs: list[dict], profile: dict, preferences: dict) -> list[dict]:
    ...
    if len(jobs) > TOTAL_JOB_CAP:
        st.session_state.setdefault("info_notices", []).append(
...
if "ranked_jobs" in st.session_state:
    ...
    for notice in st.session_state.get("info_notices", []):
        st.info(notice)
    ...
    if not ranked_jobs:
        ...
        # Arrow (which Streamlit's dataframe/export widgets rely on) can't
        # render a column that mixes Python lists with other types, and
        # missing_skills comes back from the AI as a list. Flatten it to a
        # plain comma-separated string so every render/export path is safe.
        if "missing_skills" in results_df.columns:
            results_df["missing_skills"] = results_df["missing_skills"].apply(
                lambda v: ", ".join(v) if isinstance(v, list) else (v or "")
            )
        # Guard against any other column ending up with mixed/odd types
        # (e.g. a source returning a number where a string was expected).
        for col in ["salary", "employment_type", "posting_date", "why_matches", "location", "company"]:
