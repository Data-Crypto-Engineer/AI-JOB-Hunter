...
# Note: the old "gemini-2.5-flash" name isn't reachable by new API keys
# anymore, so this uses the current-generation flash model instead.
GEMINI_MODEL: str = "gemini-3.6-flash"

MAX_QUERIES: int = 6          # how many diversified search queries the AI generates
MAX_JOBS_TO_RANK: int = 30    # cap sent to the AI ranking call, to control token usage
...
def _call_gemini_json(prompt: str, system_instruction: str) -> Any:
    ...
    fall back gracefully instead of crashing the whole search.
    """
    ...
    try:
        ...
        st.session_state.setdefault("ai_warnings", []).append(str(exc))
        return None
...
def generate_search_queries(profile: dict, preferences: dict) -> list[str]:
    ...
    system_instruction = (
        ...
        "- including adjacent and transferable roles, not just their most "
        "recent title. Respond with JSON only: "
        '{"queries": ["...", "..."]}'
    )
    prompt = (
...
def rank_and_explain_jobs(jobs: list[dict], profile: dict, preferences: dict) -> list[dict]:
    """
    Ask Gemini to score each job 0-100 against the profile, explain why
    it matches, and list missing skills. Jobs are capped at
    MAX_JOBS_TO_RANK before this call to control token usage - if there
    are more candidates than that, the highest-priority ones (first
    found) are ranked and the rest are left unscored rather than
    dropped from the results entirely.
    """
    if not jobs:
        return []

    jobs_to_rank = jobs[:MAX_JOBS_TO_RANK]
    remaining = jobs[MAX_JOBS_TO_RANK:]
    ...
    system_instruction = (
        ...
        "requires that the candidate's profile doesn't show). Respond "
        "with JSON only, matching this exact shape: "
    ...
    # Trim descriptions to keep the ranking prompt a reasonable size.
    compact_jobs = [
        {
            "index": i,
            "title": job.get("title", ""),
    ...
    if result and isinstance(result.get("results"), list):
        for entry in result["results"]:
            idx = entry.get("index")
            if isinstance(idx, int) and 0 <= idx < len(jobs_to_rank):
    ...
    for i, job in enumerate(jobs_to_rank):
        ...
        job["why_matches"] = entry.get("why_matches", "")
        job["missing_skills"] = entry.get("missing_skills", [])
        ranked.append(job)

    # Unranked overflow jobs are still shown, just without an AI score.
    for job in remaining:
        job = {**job}
        job["match_score"] = 0
        job["why_matches"] = "Not scored (over the per-search ranking limit)."
        job["missing_skills"] = []
        ranked.append(job)
